    ...
    d_ptr2(new DeviceDescriptionsPrivate)
{
    ...
    {
        DDF_FunctionDescriptor fn;
        fn.name = "air:rawtorating";
        fn.description = "Generic function to parse ZCL air quality attribute and apply selected rating for 'state/airQuality'.";
        ...
        param.name = "Scale applied to raw value";
        param.key = "to";
        param.description = "Rating the raw value is translated to for 'state/airQuality'. Currently implemented values: 'voclevelger'";
        param.dataType = DataTypeString;
        param.defaultValue = QLatin1String("voclevelger");
        param.isOptional = 1;
        param.isHexString = 0;
        param.supportsArray = 0;
        fn.parameters.push_back(param);
