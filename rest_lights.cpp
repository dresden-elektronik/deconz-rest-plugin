...
bool DeRestPluginPrivate::lightToMap(const ApiRequest &req, const LightNode *lightNode, QVariantMap &attr)
{
    ...
    for (int i = 0; i < lightNode->itemCount(); i++)
    {
        ...
        else if (rid.suffix == RStateAqaraS1PanelCommunication) { state["aqara_s1_panel_communication"] = item->toString(); }
        else if (rid.suffix == RStateAqaraS1Language) { state["aqara_s1_language"] = item->toNumber(); }
        else if (rid.suffix == RStateAqaraS1LCDBrightness) { state["aqara_s1_lcd_brightness"] = item->toNumber(); }
        else if (rid.suffix == RStateAqaraS1SoundVolume) { state["aqara_s1_sound_volume"] = item->toNumber(); }
        else if (rid.suffix == RStateAqaraS1StandbyEnabled) { state["aqara_s1_standby_enabled"] = item->toBool(); }
        else if (rid.suffix == RStateAqaraS1ScreenSaverStyle) { state["aqara_s1_screen_saver_style"] = item->toNumber(); }
        else if (rid.suffix == RStateAqaraS1Theme) { state["aqara_s1_theme"] = item->toNumber(); }
        else if (rid.suffix == RStateAqaraS1StandbyTime) { state["aqara_s1_standby_time"] = item->toNumber(); }
        else if (rid.suffix == RStateAqaraS1FontSize) { state["aqara_s1_font_size"] = item->toNumber(); }
        else if (rid.suffix == RStateAqaraS1LCDAutoBrightnessEnabled) { state["aqara_s1_lcd_auto_brightness_enabled"] = item->toBool(); }
        else if (rid.suffix == RStateAqaraS1Homepage) { state["aqara_s1_homepage"] = item->toNumber(); }
        else if (rid.suffix == RStateAqaraS1ScreenSaverEnabled) { state["aqara_s1_screen_saver_enabled"] = item->toBool(); }
        else if (rid.suffix == RStateAqaraS1StandbyLCDBrightness) { state["aqara_s1_standby_lcd_brightness"] = item->toNumber(); }
        else if (rid.suffix == RStateAqaraS1Switch1Icon) { state["aqara_s1_switch1_icon"] = item->toNumber(); }
        else if (rid.suffix == RStateAqaraS1Switch1Text) { state["aqara_s1_switch1_text"] = item->toString(); }
        else if (rid.suffix == RStateAqaraS1Switch2Icon) { state["aqara_s1_switch2_icon"] = item->toNumber(); }
        else if (rid.suffix == RStateAqaraS1Switch2Text) { state["aqara_s1_switch2_text"] = item->toString(); }
        else if (rid.suffix == RStateAqaraS1Switch3Icon) { state["aqara_s1_switch3_icon"] = item->toNumber(); }
        else if (rid.suffix == RStateAqaraS1Switch3Text) { state["aqara_s1_switch3_text"] = item->toString(); }
        else if (rid.suffix == RStateAqaraS1AutoUpdateFWEnabled) { state["aqara_s1_auto_update_fw_enabled"] = item->toBool(); }
        else if (rid.suffix == RStateAqaraS1SwitchesConfig) { state["aqara_s1_switches_config"] = item->toNumber(); }
        else if (rid.suffix == RStateAqaraS1Gestures) { state["aqara_s1_gestures"] = item->toNumber(); }
    }
...
int DeRestPluginPrivate::setLightState(const ApiRequest &req, ApiResponse &rsp)
{
    ...
    else if (taskRef.lightNode->modelId() == QLatin1String("lumi.switch.n4acn4"))
    {
        ...
        QString stringToSaveOnLight = NULL;
                ...
                if ((param == "aqara_s1_switch1_icon" || param == "aqara_s1_switch2_icon" || param == "aqara_s1_switch3_icon") && map[param].type() == QVariant::Double)
                {
                    ...
                    if (ok)
                    {
                        ...
                        if (switchText == NULL) { // In case it is null or empty...
                            ...
                            DBG_Printf(DBG_INFO, "Xiaomi switch text for 0x%2x was null\n", attr);
                        }
