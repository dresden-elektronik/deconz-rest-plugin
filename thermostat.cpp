...
QStringList weekday({"Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Away"});
TaskItem taskScheduleTimer;
int dayofweekTimer = 0;
...
void DeRestPluginPrivate::handleThermostatClusterIndication(const deCONZ::ApsDataIndication &ind, deCONZ::ZclFrame &zclFrame)
{
	...
	if (isClusterCmd && zclFrame.commandId() == 0x00)  // get schedule command response
	{
		...
		if (item && !valstored.isEmpty())
		{
			...
			foreach(QString dayEntry, schedList)
			{
				QString schedEntry;
				...
				foreach(QString checkday, checkdayList)
				{
		...
		foreach(QString key, weekdayList)
		{
		...
		foreach(QString key, mapsorted.keys())
		{
...
static QByteArray setSchedule(QString sched)
{
	QByteArray payload;
...
/*! Adds a thermostat command task to the queue.
   ...
   \return true - on success
           false - on error
 */
bool DeRestPluginPrivate::addTaskThermostatCmd(TaskItem &task, uint8_t cmd, int8_t setpoint, QString schedule, uint8_t daysToReturn)
{
 ...
 *  Iterate over every weekday and get schedule.
   ...
   \return true - on success
           false - on error
 */
bool DeRestPluginPrivate::addTaskThermostatSetAndGetSchedule(TaskItem &task, QString sched)
{
	copyTaskReq(task, taskScheduleTimer);

	if (sched != "" && !addTaskThermostatCmd(task, 0x01, 0, sched, 0))  // set schedule
	{
		return false;
	}
