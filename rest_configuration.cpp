...
int DeRestPluginPrivate::handleConfigurationApi(const ApiRequest &req, ApiResponse &rsp)
{
    ...
    // POST /api/<apikey>/config/userparameter
    else if ((req.path.size() == 4) && (req.hdr.method() == "POST") && (req.path[2] == "config") && (req.path[3] == "userparameter"))
    {
        return addUserParameter(req, rsp);
    }
    // PUT /api/<apikey>/config/userparameter
    else if ((req.path.size() == 4) && (req.hdr.method() == "PUT") && (req.path[2] == "config") && (req.path[3] == "userparameter"))
    {
        return modifyUserParameter(req, rsp);
    }
    // GET /api/<apikey>/config/userparameter
    else if ((req.path.size() == 4) && (req.hdr.method() == "GET") && (req.path[2] == "config") && (req.path[3] == "userparameter"))
    {
        return getUserParameter(req, rsp);
    }
    // DELETE /api/<apikey>/config/userparameter/<parameter>
    else if ((req.path.size() == 5) && (req.hdr.method() == "DELETE") && (req.path[2] == "config") && (req.path[3] == "userparameter"))
    {
...
void DeRestPluginPrivate::configToMap(const ApiRequest &req, QVariantMap &map)
{
    ...
    QVariantMap::const_iterator u = gwUserParameter.begin();
    QVariantMap::const_iterator uend = gwUserParameter.end();
    for (; u != uend; ++u)
    {
        map[u.key()] = u.value() ;
    }
...
int DeRestPluginPrivate::addUserParameter(const ApiRequest &req, ApiResponse &rsp)
{
    ...
    QList<QString>::const_iterator k = keys.begin();
    QList<QString>::const_iterator k_end = keys.end();

    //don't overwrite existing parameters if POST request
    for (; k != k_end; ++k)
    {
        if (gwUserParameter.contains(*k))
        {
            rsp.httpStatus = HttpStatusBadRequest;
            rsp.list.append(errorToMap(ERR_DUPLICATE_EXIST ,
            QString("config/userparameter"), QString("key %1 already exists").arg(*k)));
            return REQ_READY_SEND;
        }
    }
    ...
    for (; k != k_end; ++k)
    {
        gwUserParameter.insert(*k, map.value(*k));
        rspItemState["/config/userparameter"] = QString("%1 : %2").arg(*k).arg(map.value(*k).toString());
        rspItem["success"] = rspItemState;
        rsp.list.append(rspItem);
    }
...
int DeRestPluginPrivate::modifyUserParameter(const ApiRequest &req, ApiResponse &rsp)
{
    ...
    for (; k != k_end; ++k)
    {
        if (gwUserParameter.contains(*k))
        {
            QVariantMap::const_iterator it = gwUserParameter.find(*k);
            gwUserParameter.insert(it, *k, map.value(*k));
            rspItemState["/config/userparameter"] = QString("updated %1 : %2").arg(*k).arg(map.value(*k).toString());
            rspItem["success"] = rspItemState;
            rsp.list.append(rspItem);
        }
        else
        {
            gwUserParameter.insert(*k, map.value(*k));
            rspItemState["/config/userparameter"] = QString("added %1 : %2").arg(*k).arg(map.value(*k).toString());
            rspItem["success"] = rspItemState;
            rsp.list.append(rspItem);
        }
    }
